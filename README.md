# 📈 Catweb(Tampermonkey Edition) | 指数数据导出神器  (目前仅支持百度指数)

**Catweb** 是一个基于 Tampermonkey (油猴) 的浏览器脚本，旨在解决百度指数（Baidu Index）数据无法直接下载、只能通过肉眼查看图表的痛点。

本插件采用**非侵入式嗅探技术**，自动拦截浏览器与百度服务器之间的通信，提取加密数据与解密秘钥，在本地（浏览器端）瞬间完成解密与清洗，并一键生成带有**真实日期时间轴**的 CSV 报表。


## 视频教程：https://www.bilibili.com/video/BV1XKvZBTESg
## ✨ 核心功能 (Features)
...
4. **图标没出现？**
* 优先检查你的Extensions管理界面有没有开启develop mode保证油猴能正常读取浏览器数据
* 检查浏览器是否开启了严格模式，如果是严格模式可能会导致失败
* 检查脚本或者正常使用或开启

5. **其他问题？**
* 请反馈至issues并附带相关信息或者截图；
...
1.  **仅供科研学习**：本项目旨在展示前端数据拦截与解密算法的实现原理，仅供技术研究和学习交流使用。
2.  **严禁商业用途**：使用者**不得**将本项目用于任何商业用途（包括但不限于数据售卖、商业分析报告等），亦不得用于大规模爬虫采集或任何可能破坏目标网站（百度指数）正常运行的行为。
3.  **合法合规**：使用者在使用本项目时，必须遵守中华人民共和国相关法律法规（如《网络安全法》）。任何因违规使用本项目而导致的法律纠纷或后果，由使用者**自行承担**，项目开发者（本人）不承担任何连带责任。
4.  **数据权利**：本项目抓取的数据归属权属于百度公司。如果本项目侵犯了您的权益，请通过 [Issue] 或 [Email] 联系作者，本人将在确认后第一时间删除相关代码。
5.  **停止维护**：若百度指数官方接口发生变动或官方提出异议，本项目将可能随时停止更新或删除。
